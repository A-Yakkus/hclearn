...
def plotErrors(hist1, hist2, hist3, lost1, lost2, lost3, learningRate, subTest = True, surfTest = True, placeTest=True, note=""):
    ...
    sub_mu_1 = np.mean(hist1.sub_errs)
    sub_sigma_1 = np.std(hist1.sub_errs)/np.sqrt(T)
    sub_mu_2 = np.mean(hist2.sub_errs)
    sub_sigma_2 = np.std(hist2.sub_errs)/np.sqrt(T)
    sub_mu_3 = np.mean(hist3.sub_errs)
    sub_sigma_3 = np.std(hist3.sub_errs)/np.sqrt(T)
    ...
    if surfTest:
        # ALAN history files now have a ca1s attribute which are CA1State objects so hist1.ca1s[mapStep].surfs will
        # give the cleaned up surf features
