...
def wakeStep(ec, ca3, WB, WR, WS, WO, b_fakeSub):
    ...
    p = fuse(p,lamSenses)
    if not b_fakeSub:
        pi  = boltzmannProbs(WR,v_a3_prev)                 #probs for x next
        p = fuse(p,pi)
   ...
   # pdb.set_trace()
    if sub_fire:
        ...
        if not b_obsOnly:
            ...
            pi  = boltzmannProbs(WR,v_ca3.astype(np.float))                 #probs for x next
            p = fuse(p,pi)
...
#    v_ca3 = hstack((ca3_new.toVector(),1))
    ...
    p_odom   = boltzmannProbs(WO.transpose(), v_ca3.astype(np.float))
    p_senses = boltzmannProbs(WS.transpose(), v_ca3.astype(np.float))
